#####################################################################
#                                                                   #
# /labscript_devices/HeliCam/blacs_tabs.py                         #
#                                                                   #
...
class HeliCamTab(DeviceTab):
    ...
    the BLACS framework. Features include:
    
    - Real-time image display with PyQtGraph ImageView
    ...
    - Mode control (continuous vs. buffered)
    
    The tab manages:
    - ZMQ image reception from the worker process
    - User interaction events (buttons, dialogs)
    - Image display scaling and updates
    - Camera attribute visibility level filtering
    ...
    # different one:
    worker_class = 'labscript_devices.HeliCam.blacs_workers.HeliCamWorker' 
    ...
    def initialise_GUI(self):
        ...
        self.ui.pushButton_snap_N.clicked.connect(self.on_snap_N_clicked)
        ...
        # self.ui.lineEdit_freq.returnPressed.connect(lambda: self.update_freq())
        # self.ui.lineEdit_cycles.returnPressed.connect(lambda: self.update_cycles())
        ...
        self.auto_place_widgets(('SensTqp', ao_widgets))
        ...
        # self.ui.lineEdit_freq.setText(initial_freq)
        # self.ui.lineEdit_cycles.setText(initial_cycles)
    ...
    def on_continuous_clicked(self, button):
        ...
        self.ui.pushButton_snap_N.setEnabled(False)
    ...
    def on_stop_clicked(self, button):
        ...
        self.ui.pushButton_snap_N.setEnabled(True)
    ...
    @define_state(MODE_MANUAL, queue_state_indefinitely=True, delete_stale_states=True)
    def on_snap_N_clicked(self, button):
        n_acquisitions = self.ui.spinBox_snap_N.value()
        yield (self.queue_work(self.primary_worker, 'snap_N', n_acquisitions))
    ...
    #     yield (self.queue_work(self.primary_worker, 'program_manual', {'SensTqp' : self.ui.lineEdit_freq.text()}))
        
    # @define_state(MODE_MANUAL, queue_state_indefinitely=True, delete_stale_states=True)
    # def update_cycles(self):
    #     yield (self.queue_work(self.primary_worker, 'program_manual', {'SensNavM2' : self.ui.lineEdit_cycles.text()}))

#####################################################################
#                                                                   #
# /labscript_devices/HeliCam/blacs_workers.py                      #
#                                                                   #
...
class MockCamera(object):
    ...
    def snap_N(self, n_acquisitions, integrated_frames, settings):
        N = 300
        A = 300
        x = np.linspace(-5, 5, 300)
        y = x.reshape((N, 1))
...
class HeliCamInterface(object):
    ...
    Note:
        The HeliCam C3 sensor has a maximum resolution of 300x300 pixels. Smaller
        resolutions can be achieved through binning or ROI settings, but cannot exceed
        this maximum.
    """
    ...
    def snap_N(self, n_acquisitions, integrated_frames, settings: tuple | dict):
        """Acquire n images"""
        ...
        for i in range(ignore + n_acquisitions):
            ...
            if i < ignore:
                ...
                I = data[:, :, :, 0].sum(axis=0, dtype = np.int16)
                Q = data[:, :, :, 1].sum(axis=0, dtype = np.int16)
                # fig, axs = plt.subplots(1, 3, sharey=True)
                # fig.suptitle(f'Acquisition {i}, integrated over {integrated_frames} frames')
        ...
        return np.array([I, Q, amplitude])

    def configure_acquisition(self, continuous=True, bufferCount=5):
        """Configure camera for acquisition (buffered or continuous)"""
        try:
            # libHeLIC handles buffering internally - just ensure Acquire is ready
            print(f"Configured for acquisition (continuous={continuous}, buffers={bufferCount})")
        except Exception as e:
    ...
    def grab(self, waitForNextBuffer=True):
        ...
        try:
            ...
            print(f'Shape of {data_array.shape=}')
...
class HeliCamWorker(Worker):
    ...
    def snap(self):
        ...
        n_frames = self.camera_attributes['SensNFrames']
        self.logger.info(f"Snapping {n_frames} frames together")
        image = self.camera.snap(
            integrated_frames=self.camera_attributes["SensNFrames"],
            settings=self.camera_attributes,
        )
    ...
    def continuous_loop(self, dt):
        ...
        while True:
            ...
            try:
                ...
                self.log.error(f"Error during continuous acquisition: {e}")
                break

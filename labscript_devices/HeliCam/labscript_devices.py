#####################################################################
#                                                                   #
# /labscript_devices/HeliCam/labscript_devices.py                   #
...
Device Features:
- Hardware triggering via parent device digital output
- Configurable camera attributes for different measurement modes
- HDF5 integration for data storage and analysis
- Manual/buffered mode operation

Usage Example:
    from connection_table import parent_device
    camera = HeliCam(
        name='helicam_main',
        ...
        camera_attributes={
            'SensTqp': 4095,
            'SensNFrames': 16,
            'CamMode': 0,  # Raw IQ DF_I16Q16 format
        }
    )
"""
...
class HeliCam(TriggerableDevice):
    ...
    The HeliCam C3 has a fixed maximum sensor resolution of 300x300 pixels.
    Smaller resolutions can be achieved through binning or ROI settings.
    """
    ...
    def get_image_dimensions(self):
        """Get the maximum image dimensions for HeliCam C3.
        
        The HeliCam C3 sensor has a maximum resolution of 300x300 pixels.
        Smaller resolutions can be achieved through binning or ROI settings.
        
        Returns:
            tuple: (max_width, max_height) in pixels, always (300, 300)
        """
